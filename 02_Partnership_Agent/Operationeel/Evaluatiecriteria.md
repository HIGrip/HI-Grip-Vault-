# Evaluatiecriteria — HÏ Grip Influencer Zoek Agent

> Bijgewerkt: 2026-06-24
> Zie ook: [[Influencer Database]] · [[Zoek Script & Gids]] · [[Ideale Partner Profiel]]

---

## Niche / Content type (prioriteit)

| Prioriteit | Sport / Niche |
|---|---|
| ⭐ Hoog | Tennis |
| ⭐ Hoog | Padel |
| ⭐ Hoog | Voetbal (street, panna, zaal) |
| Normaal | Urban sports / streetball |
| Normaal | Basketball |
| Normaal | Rugby |
| Normaal | Gym / fitness lifestyle |
| Normaal | Hardlopen / running |
| Normaal | Pilates / yoga |

---

## Platform

| Platform | Prioriteit |
|---|---|
| Instagram | ⭐ Hoog |
| TikTok | ⭐ Hoog |

---

## Bereik — metrics

> **Metric:** VIEWS per post (niet alleen volgers)

| Tier | Volgersband | Views per post |
|---|---|---|
| Nano- | 500–2.000 | 500–2.000 |
| Nano | 2.000–5.000 | 2.000–5.000 |
| **Nano/micro ⭐** | **5.000–30.000** | **5.000–30.000** |
| Micro | 30.000–100.000 | 30.000–100.000 |
| Mid-tier | 100.000–500.000 | 100.000–500.000 |

- **Minimum volgers:** 500
- **Maximum volgers:** 100.000
- **Minimum activiteit:** 3 posts in de afgelopen 3 weken

---

## Locatie

- Nederland ⭐
- België ⭐

---

## Engagement Rate

- Minimum: 2%
- Voorkeur: 5–10%

---

## Kwaliteitseisen

- Echte volgers (geen fake engagement / gekochte volgers)
- Actief: minimaal 3 posts per 3 weken
- Taal: Nederlands
- Professionele uitstraling = GEEN harde eis — iPhone + goede connectie met volgers is ook goed
- Past bij HÏ Grip imago: stoer, motiverend, performance-gedreven (zie [[Brand Identity Overview]])

---

## Type samenwerking (opties)

- Gifting
- Betaalde post
- Affiliate
- Brand ambassador

---

## Uitsluitingen

- Geen sport/lifestyle focus
- Slechte contentkwaliteit
- Gekochte volgers / fake engagement
- Al benaderd (checken via [[Pipeline Tracker]])
- Promoot concurrerende grip sock merken

---

## Verificatie-checklist per account

| Criterium | Wat checken | Goed | Afwijzen |
|---|---|---|---|
| Volgers | Tel exact | Max. 100.000 volgers | Meer dan 100.000 |
| Views | Laatste 5 videos | >5K views consistent | 1 viral hit, rest laag |
| Content-stijl | Engaging? Wil je doorkijken? | Eigen stijl, eigen stem, verhaal | Compilatie, geen persoon |
| Persoonlijk | Eigen gezicht in iedere video? | Creator IS de content | Alleen reposts van anderen |
| Video-first | Verhouding video vs foto | >70% reels / video | Voornamelijk fotoposts |
| Actief | Laatste post datum | Gepost <3 weken geleden | Laatste post >6 weken geleden |
| NL publiek | Taal comments + bio | Nederlands of duidelijke NL connectie | Volledig internationaal |

---

## Zoekstrategie (volgorde)

1. Hashtag zoeken op Instagram / TikTok (zie [[Zoek Script & Gids]])
2. Zoek direct op de platforms
3. Volgers van concurrerende merken checken
4. Volgers van HÏ Grip eigen account analyseren
5. Volgers van grote sport accounts in niche
6. Following list bekende sport-influencers + tagged posts niche-events

---

## Output per creator (vast format)

Bij toevoegen aan [[Influencer Database]]:

- Naam (klikbare link naar profiel)
- Platform
- Volgersaantal
- Gemiddelde views per post
- ER%
- Niche + locatie
- Status: Nieuw / Verifieer / Gevonden / Benaderd / In gesprek / Actief

---

## Werkafspraken

- Agent innoveert als strategie niet werkt — maar stelt voorstel voor, past niet zelf aan
- Focus op content creators, niet per se influencers
