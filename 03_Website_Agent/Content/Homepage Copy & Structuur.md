# Homepage Copy & Structuur — HÏ Grip

> Baseline + psychologie-audit vastgelegd 2026-07-14 op basis van live www.higrip.nl. Voor tone of voice: zie [[Brand Voice & Tone of Voice]]. Voor bredere actiepunten: zie [[Conversie Optimalisatie Checklist]].

> **Beachhead-strategie (vanaf 16-9-2026):** HÏ Grip focust nu op tennis, rugby en voetbal (zie [[Doelgroep & Persona's]]). Concreet effect op deze pagina: sectie 9 "Sport-specifieke voordelen" moet deze 3 sporten prioriteren/uitlichten i.p.v. breed alle sporten tonen. Overige secties (banner, kernwaarden, FAQ) blijven merk-breed — zie de open vraag in [[SEO Strategie & Keywords]] over hoe ver deze focus in de sitestructuur doorgetrokken wordt.

---

## Huidige opbouw (baseline)

1. Navigatie & taalkeuze
2. Promotionele banner — "2+2 GRATIS", WK 2026-thema, tagline "Heel Nederland kijkt. Jij scoort dubbel.", CTA "Scoor nu je 2+2→"
3. Hoe het werkt (3 stappen)
4. Productvoordelen & Trustpilot 4,5/5
5. Prijsopbouw: €14,99/paar, actie €29,98 voor 4 paar (normaal €59,96)
6. Kernwaarden: Comfort, Vertrouwen, Innovatie (de merk-driehoek — zie [[Logo & Kleurenpalet]])
7. "Ons verhaal" teaser
8. Productshowcase
9. Sport-specifieke voordelen
10. Team HÏ Grip introductie
11. Zakelijke oplossingen
12. Partner-logo's
13. FAQ (6 vragen incl. wetenschappelijke bronnen)
14. Footer

---

## Psychologie-audit

**Systeem 1 vs Systeem 2 (Kahneman)**
Homepage opent goed op Systeem 1: actie/promotie, tagline, visuele banner — vóór er specificaties komen. Dat is de juiste volgorde. Kernwaarden-sectie (Comfort/Vertrouwen/Innovatie) en FAQ met wetenschappelijke bronnen bedienen Systeem 2 verderop — ook correct gepositioneerd (later in de pagina, ná de emotionele hook).

**Cialdini — aanwezig**
- *Sociale bewijskracht*: Trustpilot 4,5/5, klantenaantal-claim, partner-logo's
- *Schaarste*: actie loopt af (WK-finale periode), countdown-achtige framing
- *Autoriteit*: wetenschappelijke bronnen in FAQ (3 citaties)
- *Eenheid*: #TEAMHÏGRIP, "Team HÏ Grip"-sectie

**Cialdini — zwak/ontbrekend**
- *Consistentie*: klantenaantal-claim homepage (2.000+) wijkt af van productpagina (1500+) — dit ondermijnt juist de sociale bewijskracht die de rest van de pagina opbouwt. Congruentie tussen claims is een voorwaarde voor vertrouwen, niet een detail.
- *Autoriteit* staat nu ver onderaan (FAQ) — de wetenschappelijke onderbouwing zou eerder in de pagina (bij de productvoordelen-sectie, punt 4) al kunnen worden aangestipt, niet pas achteraan.

**Aanbeveling (voorstel, nog niet doorgevoerd)**
1. Eén klantenaantal-cijfer overal aanhouden
2. Korte autoriteit-cue (bv. "wetenschappelijk onderbouwd") al bij sectie 4 laten zien, met verwijzing naar de volledige FAQ-onderbouwing

---

## Gerelateerde bestanden

- [[Brand Voice & Tone of Voice]] — Slogans en verbale stijl
- [[Website Structuur & Sitemap]] — Volledige sitemap
- [[Conversie Optimalisatie Checklist]] — Actiepunten
