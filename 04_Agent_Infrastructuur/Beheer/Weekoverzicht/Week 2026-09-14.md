...
- Funnel: 42x `view_item`, 6x `add_to_cart`, 7x `begin_checkout`, 2x `add_shipping_info` — maar **0 `purchase`-events en €0 omzet geregistreerd in GA4**, ook deze week weer. Dit bevestigt het al langer openstaande punt dat purchase-events niet doorkomen in GA4 (zie "Analytics-vervolgstappen" hierboven, nu al 3+ weken open) — dit is dus geen toevallige dip maar een structureel trackingprobleem dat prioriteit verdient, los van de daadwerkelijke omzet (die wel via Shopify zelf te zien is).
- Kanttekening: dit is pas de tweede week met vergelijkbare data sinds de GA4-tag weer actief is, dus nog te vroeg voor trendconclusies — vooral de purchase-tracking is het concrete actiepunt.
