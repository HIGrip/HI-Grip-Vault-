...
- **Zoekmethode — Overpass API (gratis, nieuw 2026-09-17):** lokale bedrijvenlijsten komen uit **OpenStreetMap via de Overpass API**. Geen API-key, geen account, geen kosten; ~10.000 requests per dag. Dit verving een betaalde Apify-skill die feitelijk een wrapper om dezelfde OSM-data was — dus geen functieverlies, alleen geen rekening.
  ...
  **Getest en werkend (17-9):** één query leverde **71 sportscholen in regio Rotterdam**, waarvan 51 met website en 19 met telefoonnummer.

  ```bash
  curl -s -A "HiGrip-LeadResearch/1.0 (info@higrip.nl)" \
    "https://overpass.kumi.systems/api/interpreter" \
    --data-urlencode 'data=[out:json][timeout:25];(node["leisure"="fitness_centre"](ZUID,WEST,NOORD,OOST);way["leisure"="fitness_centre"](ZUID,WEST,NOORD,OOST););out body;'
  ```

  **Twee dingen die je moet weten of het faalt:** (1) stuur **altijd een eigen User-Agent** mee — zonder UA geeft de server `406 Not Acceptable`; (2) de hoofdserver `overpass-api.de` is regelmatig overbelast ("server is probably too busy") — gebruik dan de mirror **`overpass.kumi.systems`**, die tijdens de test wel werkte.

  **Relevante OSM-tags voor HÏ Grip:** `leisure=fitness_centre` (sportscholen), `leisure=sports_centre` (sportcomplexen), `club=sport` (sportverenigingen), `shop=sports` (sportwinkels/retailers). Elk resultaat geeft `name`, `website`, `phone`, `opening_hours` en adresvelden.
