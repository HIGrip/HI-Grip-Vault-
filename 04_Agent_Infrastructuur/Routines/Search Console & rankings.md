...
7. **Doorwerking:** zoek in `_geheugen/seo-conversietest.md` welke pagina's zijn aangepast, en rapporteer hun trend sinds de startdatum.

Te weinig data (bijvoorbeeld minder dan 100 vertoningen per zoekterm)? Zeg dat en trek geen conclusies.

## Stap 3 — Output
1. **Notitie** `05_Research/JJJJ-MM-DD-search-console.md`: `bron: routine`, `routine: search-console`, `categorie: SEO`. Inhoud:
   - kerncijfers-tabel (deze week, vorige week, verschil)
   - tabel met kernkeywords (positie, verschil, URL)
   - nieuwe zoektermen
   - kansen (striking distance, lage CTR)
   - kannibalisatie
   - doorwerking van eerdere verbeteringen
2. **Backlog:** maximaal 3 nieuwe punten per week, alleen concrete kansen (bijvoorbeeld "titel /collections/gripsokken herschrijven: 340 vertoningen, CTR 0,6%"). Prefix `[search-console]`.
